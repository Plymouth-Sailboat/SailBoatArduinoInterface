...
function build_platform()
{
  ...
  if [ "$PLATFORM_CHECK_ONLY_ON_FILE" = true ]; then
    ...
    examples=($(
      for f in $(find . -type f -iname 'AutonomousSailboat.ino' -o -iname 'AutonomousSailboat.pde'); do
    ...
    # loop through results and add them to the array
    examples=($(find $PWD -name "AutonomousSailboat.pde" -o -name "AutonomousSailboat.ino"))
  fi
